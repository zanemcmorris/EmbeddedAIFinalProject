  ...
  *new_gyroscope_samples = 0;
  ...
  while (lsm6ds3trc.accelerationAvailable()) {
    ...
    *new_gyroscope_samples += 1;
    ...
    // Read each sample, removing it from the device's FIFO buffer
    if (!lsm6ds3trc.readAcceleration(
...
int ReadGyroscope() {
  ...
  while (lsm6ds3trc.gyroscopeAvailable()) {
    ...
    // Read each sample, removing it from the device's FIFO buffer
    if (!lsm6ds3trc.readGyroscope(data[0], data[1], data[2])) {
      Serial.println("Failed to read gyroscope data");
      // break;
    }
    new_samples += 1;
...
void loop() {
  ...
  const bool data_available = lsm6ds3trc.accelerationAvailable() || lsm6ds3trc.gyroscopeAvailable();
  if (!data_available) {
    return;
  }
